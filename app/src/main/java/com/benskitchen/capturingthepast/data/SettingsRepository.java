...
public class SettingsRepository {
    ...
    public String getArchonAt(int index) {
        ...
        return archon.isEmpty() ? DEFAULT_ARCHON : archon;
    }
    ...
    public void addRepository(String strRepo, String strArchon) {
        ...
        list.put(jsonObj);
        int len = getRepositories().length();
        if (len > 0) {
            try {
                for (int i = 0; i < len; i++) {
                    list.put(getRepositories().get(i));
                }
            } catch (JSONException e) {
                e.printStackTrace();
            }
    ...
    public String deleteRepository(int n) {
        try {
            JSONArray list = new JSONArray();
            ...
            jsonObj = getRepositories().getJSONObject(n);
            return jsonObj.getString("Repository");

        } catch (JSONException e) {
            e.printStackTrace();
        }
        return "No repository deleted.";
    }
